...
const RangePickerComponent = ({option, dateFn}) => {
    ...
    const antdRangePickerFooter = () => {
        if(option.startDate === undefined || option.endDate === undefined){ return (<><p>날짜를 선택해주세요.</p></>) } 
        else {
    ...
    return (
        ...
        <div className="ant-input-range-cover">
            {buttonListArray.map((active, index) => {
                return active === "1" ? 
                <Button key={index} type="primary" danger onClick={()=> { dateFn.startFn(buttonFns[index].start); dateFn.endFn(buttonFns[index].end); }}>{buttonFns[index].name}</Button>
                : null
            })}
